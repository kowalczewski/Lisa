...
def J_en(en, ref_indx, alpha_LT, am15g, V):
    '''
    Returns 3 currents (base, emitter, SRC) for a given V and energy.
    '''
    ...
    spect_fact = am15g/en / solar_spect_int
    ...
    # 1/q: W --> eV
    # 1E-4: 1/m2 --> 1/cm2 
    a_minus = alpha_LT * am15g/en / (1 - F1 * (1-1/ref_indx**2) ) * 1/q * 1E-4 
    a_plus = a_minus * Rb
    ...
    # Units '1E3': A --> mA.
    return J_B * 1E3, - J_E * 1E3, J_scr * 1E3
        
# Returns total current as a function of voltage.
def J_v(V):
    
    # Here J_en() will return 3 vectors. The function is executed for these vectors:
    # en_vec, ref_indx_vec, alpha_LT_vec, am15g_vec,
    # all of them have the same size.
    J_B_vec, J_E_vec, J_scr_vec = J_en(en_vec, ref_indx_vec, alpha_LT_vec, am15g_vec, V)
...
if __name__ == "__main__":
    ...
    if (params.jv_flag):
        ...
        # Drop voltage values above Vbi (built-in potential).
        # v_vec = v_vec[v_vec<=Vbi]
        ...
        optimization = opt.minimize_scalar(lambda v: -J_v(v)*v)
        ...
        Voc = opt.brenth(lambda v: J_v(v), Vm, 1)
